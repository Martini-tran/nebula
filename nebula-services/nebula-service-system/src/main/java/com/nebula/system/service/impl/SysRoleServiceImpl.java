...
public class SysRoleServiceImpl implements SysRoleService {
    ...
    /**
     * roleCode 仅允许字母 / 数字 / 下划线 / 短横线，长度 2-50
     */
    ...
    public RoleListVO detail(Long id) {
        ...
        if (role == null) {
            throw new BizException(SystemResultCode.ROLE_NOT_FOUND);
        }
    ...
    public Long create(RoleCreateRequest req) {
        ...
        if (!notBlank(req.getRoleName())) {
            throw new BizException(SystemResultCode.ROLE_NAME_EXISTS.getCode(), "请输入角色名称");
        }
        if (existsByCode(req.getRoleCode(), null)) {
            throw new BizException(SystemResultCode.ROLE_CODE_EXISTS);
        }
        if (existsByName(req.getRoleName(), null)) {
            throw new BizException(SystemResultCode.ROLE_NAME_EXISTS);
        }
    ...
    public void delete(Long id) {
        ...
        if (boundUsers != null && boundUsers > 0) {
            throw new BizException(SystemResultCode.ROLE_HAS_USERS);
        }
        // 清理菜单关联
        roleMenuMapper.delete(new LambdaQueryWrapper<SysRoleMenu>()
                .eq(SysRoleMenu::getRoleId, id));
        // 同 SysUser.delete 一致：先标记 delete_time，再触发 @TableLogic
        SysRole patch = new SysRole();
        patch.setId(id);
    ...
    public void updateStatus(Long id, Integer status) {
        if (status == null || (status != 0 && status != 1)) {
            throw new BizException(SystemResultCode.ROLE_NOT_FOUND.getCode(), "状态值非法");
        }
        SysRole current = roleMapper.selectById(id);
        if (current == null) {
            throw new BizException(SystemResultCode.ROLE_NOT_FOUND);
    ...
    private void validateCode(String code) {
        if (!notBlank(code)) {
            throw new BizException(SystemResultCode.ROLE_CODE_INVALID, "请输入角色编码");
        }
        if (!code.matches(ROLE_CODE_REGEX)) {
            throw new BizException(SystemResultCode.ROLE_CODE_INVALID,
