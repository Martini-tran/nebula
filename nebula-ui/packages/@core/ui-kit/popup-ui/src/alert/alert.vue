...
import { globalShareState } from '@nebula-core/shared/global-state';
      ...
      <div :class="cn('relative flex-1 overflow-y-auto p-3', contentClass)">
        ...
        <AlertDialogDescription>
          <div class="m-4 min-h-7.5">
            <nebulaRenderContent :content="content" render-br />
          </div>
          <nebulaLoading v-if="loading && contentMasking" :spinning="loading" />
        </AlertDialogDescription>
        <div
          class="flex items-center justify-end gap-x-2"
          :class="`justify-${buttonAlign}`"
        >
          <nebulaRenderContent :content="footer" />
          <AlertDialogCancel v-if="showCancel" as-child>
            <component
              :is="components.DefaultButton || nebulaButton"
              :disabled="loading"
              variant="ghost"
              @click="handleCancel"
            >
          ...
          <AlertDialogAction as-child>
            <component
              :is="components.PrimaryButton || nebulaButton"
              :loading="loading"
              @click="handleConfirm"
            >
