...
import { nebulaRenderContent } from '@nebula-core/shadcn-ui';
...
declare global {
  const __nebula_ADMIN_METADATA__: {
    authorEmail: string;
    authorName: string;
    authorUrl: string;
    buildTime: string;
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'nebula-link' },
    { default: () => text },
  );
...
} = __nebula_ADMIN_METADATA__ || {};

const nebulaDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: '版本号',
  },
  ...
  {
    content: renderLink(nebula_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: renderLink(nebula_PREVIEW_URL, '点击查看'),
    title: '预览地址',
  },
  {
    content: renderLink(nebula_GITHUB_URL, '点击查看'),
    title: 'Github',
  },
  {
    content: h('div', [
  ...
  <Page :title="title">
    <template #description>
      <p class="mt-3 text-sm/6 text-foreground">
        <a :href="nebula_GITHUB_URL" class="nebula-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
    ...
    <div class="card-box p-5">
      ...
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in nebulaDescriptionItems" :key="item.title">
            <div class="border-t border-border px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="mt-1 text-sm/6 text-foreground sm:mt-2">
                <nebulaRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>
