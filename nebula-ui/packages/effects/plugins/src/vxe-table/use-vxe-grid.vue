...
import type { nebulaFormProps } from '@nebula-core/form-ui';
...
import { usePriorityValues } from '@nebula/hooks';
import { EmptyIcon } from '@nebula/icons';
...
async function init() {
  ...
  // form 由 nebula-form代替，所以不适配formConfig，这里给出警告
  const formConfig = gridOptions.value?.formConfig;
  // 处理某个页面加载多个Table时，第2个之后的Table初始化报出警告
  // 因为第一次初始化之后会把defaultGridOptions和gridOptions合并后缓存进State
  if (formConfig && formConfig.enabled) {
    console.warn(
      '[nebula Vxe Table]: The formConfig in the grid is not supported, please use the `formOptions` props',
    );
  }
  props.api?.setState?.({ gridOptions: defaultGridOptions });
  // form 由 nebula-form 代替，所以需要保证query相关事件可以拿到参数
...
watch(
  ...
  () => {
    formApi.setState((prev: Record<string, any>) => {
      const finalFormOptions: nebulaFormProps = mergeWithArrayOverride(
        {},
        formOptions.value,
        prev,
      );
      ...
      <template v-if="showToolbar" #toolbar-actions="slotProps">
        <slot v-if="showTableTitle" name="table-title">
          <div class="flex-center gap-1 text-[1rem] font-bold">
            {{ tableTitle }}
            <nebulaHelpTooltip v-if="tableTitleHelp">
              {{ tableTitleHelp }}
            </nebulaHelpTooltip>
          </div>
        </slot>
