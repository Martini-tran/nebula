...
import type { AnyFunction } from '@nebula/types';

import { computed, useTemplateRef, watch } from 'vue';

import { useHoverToggle } from '@nebula/hooks';
import { LockKeyhole, LogOut } from '@nebula/icons';
...
import { isWindowsOs } from '@nebula/utils';
...
const [LockModal, lockModalApi] = usenebulaModal({
  connectedComponent: LockScreenModal,
});
const [LogoutModal, logoutModalApi] = usenebulaModal({
  onConfirm() {
    handleSubmitLogout();
  },
});
    ...
    <DropdownMenuContent class="mr-2 min-w-60 p-0 pb-1">
      <div ref="refContent">
        ...
        <DropdownMenuItem
          ...
          @click="menu.handler"
        >
          <nebulaIcon :icon="menu.icon" class="mr-2 size-4" />
          {{ menu.text }}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem
