...
import type {
  ...
  ToolbarAction,
  ToolbarMenuItem,
  nebulaTiptapChangeEvent,
} from './types';

import { computed, onBeforeUnmount, reactive, watch } from 'vue';

import { Check, ChevronDown, Eye } from '@nebula/icons';
import { $t } from '@nebula/locales';

import { usenebulaModal } from '@nebula-core/popup-ui';
import { nebulaIconButton, nebulaPopover } from '@nebula-core/shadcn-ui';
...
const emit = defineEmits<{
  change: [payload: nebulaTiptapChangeEvent];
}>();
...
const tiptapContentClass = cn(
  'nebula-tiptap-content nebula-tiptap__content',
  'text-foreground min-h-(--nebula-tiptap-min-height) leading-7 outline-none',
);
...
const [PreviewModal, previewModalApi] = usenebulaModal({
  footer: false,
...
<template>
  <div
    :style="{ '--nebula-tiptap-min-height': contentMinHeight }"
    class="nebula-tiptap overflow-hidden rounded-xl border border-border bg-card"
  >
    <div
      v-if="toolbar"
...
<style scoped>
.nebula-tiptap
  :deep(.nebula-tiptap__content p.is-editor-empty:first-child::before) {
  float: left;
  height: 0;
