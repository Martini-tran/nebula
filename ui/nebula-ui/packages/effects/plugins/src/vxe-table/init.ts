...
export function useTableForm(...args: any[]) {
  ...
  const contextFormFactory = pluginsOptions?.form?.usenebulaForm;

  const factory = tableFormFactory || contextFormFactory;
  if (!factory) {
    throw new Error(
      'useTableForm is not initialized. Please provide usenebulaForm via setupnebulaVxeTable() or providePluginsOptions()',
    );
  }
...
export function setupnebulaVxeTable(setupOptions: SetupVxeTable) {
  const { configVxeTable, usenebulaForm: usenebulaFormFromParam } = setupOptions;

  initVxeTable();

  // 优先使用参数传入的 usenebulaForm，否则清空让 context 注入生效
  if (usenebulaFormFromParam) {
    tableFormFactory = usenebulaFormFromParam;
  }
