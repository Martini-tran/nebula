...
import type { ValueType, nebulaButtonGroupProps } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@nebula-core/icons';
import { cn, isFunction } from '@nebula-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { nebulaRenderContent } from '../render-content';
import nebulaButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<nebulaButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
...
<template>
  <nebulaButtonGroup
    :size="props.size"
    :gap="props.gap"
    class="nebula-check-button-group"
  >
    <Button
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <nebulaRenderContent :content="btn.label" />
      </slot>
    </Button>
  </nebulaButtonGroup>
</template>
<style lang="scss" scoped>
.nebula-check-button-group {
  display: flex;
  flex-wrap: wrap;
