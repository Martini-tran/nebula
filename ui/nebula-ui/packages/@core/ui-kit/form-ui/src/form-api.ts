...
import type { FormActions, FormSchema, nebulaFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@nebula-core/shared/store';
import {
  bindMethods,
  cloneDeep,
  createMerge,
  ...
  set,
  StateHandler,
} from '@nebula-core/shared/utils';

import { resolveFieldNamePath } from './field-name';

function getDefaultState(): nebulaFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<nebulaFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | nebulaFormProps = null;
  stateHandler: StateHandler;

  public store: Store<nebulaFormProps>;
  ...
  private prevState: null | nebulaFormProps = null;

  constructor(options: nebulaFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<nebulaFormProps>({
      ...defaultState,
      ...storeState,
    });
  ...
  setState(
    stateOrFn:
      | ((prev: nebulaFormProps) => Partial<nebulaFormProps>)
      | Partial<nebulaFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<nebulaForm /> is not mounted');
    }
    return this.form;
  }
