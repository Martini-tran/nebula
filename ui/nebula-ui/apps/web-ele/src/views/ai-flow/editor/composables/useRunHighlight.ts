/**
 * 运行结果回放高亮：根据 node_results 在画布上标记走过/跳过的节点。
 *
 * 后端 run 是同步一次性返回（非流式），故这里做「运行完成后回放」而非
 * 实时逐节点点亮。node_results 的 key 集合 = 本次实际执行的节点。
 *
...
export function useRunHighlight(options: UseRunHighlightOptions) {
  ...
  /** 根据运行结果打高亮：命中 node_results → executed，未命中 → skipped */
  function applyRunResult(result: AiFlowApi.FlowRunResultRaw) {
    const g = getGraph();
    if (!g) return;
    const ran = new Set(Object.keys(result.node_results ?? {}));
    withoutHistory(() => {
